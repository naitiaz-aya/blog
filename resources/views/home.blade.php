...
<body>
    <div id="app">
            ...
            <div class="container text-center">
                <div class="row">
                    <div class="col-md-7 col-sm-12 bck">
                        ...
                        @auth
                        @if(Auth::user()->role == 'staff')
                        <a href="{{route('addBlog')}}"><button class="btn btn-clo px-5 py-2 primary-btn">
                          Add New Blog
                        </button></a>
                        @endif
                        @if(Auth::user()->role == 'normal')
                        <a href="{{route('addArticle')}}"><button class="btn btn-clo px-5 py-2 primary-btn">
                          Write Your Article
                        </button></a>
                        @endif
                        @endauth
                        @auth
                        @if(Auth::user()->role == 'admin')
                        <a href="{{route('dashboard')}}"><button class="btn btn-clo px-5 py-2 primary-btn">
                          Go To Dashboard
                        </button></a>
                        @endif
                        @endauth
        ...
        <button type="button" class="close btn btn-transparent" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">×</span></button>
            <p>{{ $message }}</p>
        </div>
    @endif
        @auth
        @if(Auth::user()->role == 'staff')
        <a href="{{route('addCategory')}}"><button class="btn btn-success px-5 py-2 primary-btn mt-2">
          Add a Category
        </button></a>
        @endif
        @endauth
            ...
            <div class="row">
              ...
              <div class="col-12">
                <div class="media-body u-shadow-v18 g-bg-secondary g-pa-30">
                  ...
                  <div class="g-mb-15">
                    <img class="postimg" src="{{ Storage::url($blog->image) }}"  alt="post" />
                    <div class="d-flex justify-content-end">
                      ...
                      @auth
                        ...
                        <form action="{{ route( 'deleteBlog', ['id' => $blog->id])}}" method="post" >
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-danger m-1"><i class="fas fa-backspace"></i></button>
                        </form>
                        <a href="{{ route( 'editBlog', ['id' => $blog->id])}}"><button class="btn btn-primary m-1"><i class="fas fa-edit"></i></button></a>
                        @endif
                    @endauth
                    </div>
                    <h5 class="h5 g-color-gray-dark-v1 mb-0">{{ $blog->user->fname }} </h5>
                    <span class="g-color-gray-dark-v4 g-font-size-12">{{ $blog->created_at }}</span>
                  </div>
                  <a href="{{ route('showBlog', ['id' => $blog->id])}}" class="h3">{{$blog->title}} </a>
                  <p>{{ $blog->body }}</p>
